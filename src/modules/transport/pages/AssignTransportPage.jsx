...
import { Select, message, Modal, Button, Upload } from "antd";
import {
...
const AssignTransportPage = ({ initialStudentId, onMounted }) => {
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      try {
        ...
        const allStudents = admissionsResponse.data || [];
  ...
  const handleBulkUpload = async () => {
    if (!bulkFile) { message.error("Please select a file"); return; }
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        // Map keys to match BulkAssignItemDto
        const items = json.map(row => ({
          admissionNo: String(row.admissionNo || row['Admission No'] || row['AdmissionNo'] || ''),
          routeNo: String(row.routeNo || row['Route No'] || row['RouteNo'] || ''),
          busNo: String(row.busNo || row['Bus No'] || row['BusNo'] || ''),
          stopName: String(row.stopName || row['Stop Name'] || row['StopName'] || ''),
        })).filter(item => item.admissionNo && item.routeNo);

        if (items.length === 0) {
          message.error("No valid data found in file. Ensure columns: admissionNo, routeNo, busNo, stopName");
          setUploadingBulk(false);
          return;
        }

        const res = await bulkAssignTransport({
          academicYear,
          items
        });

        if (res.failed === 0) {
          message.success(`Successfully uploaded ${res.success} assignments!`);
        } else {
          message.warning(`Uploaded ${res.success} assignments. ${res.failed} failed.`);
  ...
  // apply standard + section + archived filters
  const filteredStudents = students.filter((s) => {
    if (!showArchived && s.users?.isActive === false) return false;
  ...
  const studentOptions = filteredStudents.map((s) => {
    ...
    const archivedLabel = s.users?.isActive === false ? " [ARCHIVED]" : "";
    return {
      value: s.id,
      label: `${s.name}${section} — ${standardLabel} — ${admNo}${archivedLabel}`,
      searchText: `${s.name} ${standardLabel} ${s.standard} ${s.section || ""} ${admNo} ${archivedLabel}`.toLowerCase(),
    };
  });
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          {/* Academic Year Selection moved to top */}
          <div className="space-y-1">
            ...
            <div className="relative min-w-[140px]">
              <select
                ...
                className="w-full px-4 py-2.5 bg-white shadow-sm border-none rounded-xl font-bold text-primary appearance-none focus:ring-2 focus:ring-primary/20 outline-none cursor-pointer text-sm"
              >
                {availableYears.map((year) => (
                  <option key={year} value={year}>{year}</option>
                ))}
              </select>
              <span className="material-symbols-outlined absolute right-2 top-2.5 pointer-events-none text-primary text-base">expand_more</span>
            </div>
          </div>

          <button
            onClick={() => setBulkModalOpen(true)}
            className="px-6 py-3 bg-primary text-white font-bold text-sm rounded-xl shadow-lg hover:shadow-xl active:scale-95 transition-all flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-7 items-start">
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl shadow-ambient overflow-hidden">
          ...
          <div className="p-8 lg:p-10">
            ...
            <div className="space-y-5">
              {/* ── Filter row: Standard + Section + Archived ── */}
                <div className="grid grid-cols-3 gap-3">
                  ...
                  {/* Archived Toggle */}
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider ml-1 text-center block">Archived</label>
                    <div className="flex items-center justify-center h-[42px] bg-surface-container-high rounded-xl">
                      <button
                        onClick={() => { setShowArchived(!showArchived); setStudentId(null); }}
                        className={`flex items-center gap-2 px-4 py-1.5 rounded-lg transition-all ${showArchived ? 'bg-primary text-white' : 'text-on-surface-variant hover:bg-white/50'}`}
                      >
                        <span className="material-symbols-outlined text-sm">{showArchived ? 'visibility' : 'visibility_off'}</span>
                        <span className="text-[10px] font-bold uppercase">{showArchived ? 'On' : 'Off'}</span>
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full px-4 py-3.5 bg-surface-container-high border-none rounded-xl font-body text-on-surface appearance-none focus:ring-2 focus:ring-primary/20 outline-none cursor-pointer text-sm disabled:opacity-50"
                    ...
                    >
                      ...
                      {routeBuses.map((bus) => (
                        <option key={bus.id} value={bus.number}>{bus.number} (Cap: {bus.capacity})</option>
                      ))}
                    </select>
                    <span className="material-symbols-outlined absolute right-3 top-3.5 pointer-events-none text-on-surface-variant text-base">expand_more</span>
                  </div>
            ...
            <div className="bg-white shadow-[0_20px_40px_rgba(1,29,53,0.15)] p-4 rounded-full flex items-center gap-4 border border-outline-variant/10">
              ...
              <div className="flex -space-x-3 ml-2">
                {["SC", "RT", `+${Math.max(0, students.length - 2)}`].map((label, i) => (
                  <div
                    key={i}
                    className={`w-8 h-8 rounded-full border-2 border-white flex items-center justify-center text-[10px] font-bold text-primary ${i === 0 ? "bg-primary-fixed" : i === 1 ? "bg-secondary-container" : "bg-surface-container-highest"
                      }`}
                  >
                    {label}
                  </div>
        ...
        {/* ── Bulk Upload Modal ── */}
        <Modal
          title={<div className="flex items-center gap-2"><span className="material-symbols-outlined text-primary">upload_file</span><span>Bulk Transport Assignment</span></div>}
          open={bulkModalOpen}
          onCancel={() => setBulkModalOpen(false)}
          footer={[
            <Button key="cancel" onClick={() => setBulkModalOpen(false)}>Cancel</Button>,
            <Button
              key="upload"
              type="primary"
        ...
        >
          <div className="space-y-4 py-4">
            <div className="p-4 bg-primary/5 rounded-xl border border-primary/10">
              <h4 className="text-sm font-bold text-primary mb-1">Upload Instructions</h4>
              <p className="text-xs text-on-surface-variant leading-relaxed">
                Upload an Excel (.xlsx) or CSV file with the following columns:
                <br />
                <code className="bg-white px-1 py-0.5 rounded text-primary font-bold">admissionNo</code>,
                <code className="bg-white px-1 py-0.5 rounded text-primary font-bold">routeNo</code>,
                <code className="bg-white px-1 py-0.5 rounded text-primary font-bold">busNo</code>,
                <code className="bg-white px-1 py-0.5 rounded text-primary font-bold">stopName</code> (optional)
              </p>
            </div>
            ...
            >
              ...
              <p className="ant-upload-hint">Support for single .xlsx or .csv files only.</p>
            </Upload.Dragger>

            <p className="text-[10px] text-center text-on-surface-variant">
              Assignments will be mapped to the academic year: <span className="font-bold text-primary">{academicYear}</span>
            </p>
          </div>
        </Modal>
      </div>
