...
const formatStandardLabel = (standard) => {
  const match = String(standard || '').match(/^STD_(\d{1,2})$/);
...
const PromotionPage = () => {
  ...
  const onPromote = async () => {
    try {
      ...
      const actionLabel = isDemotion ? 'Demotion' : 'Promotion';
      ...
      confirm({
        ...
        content: (
          <div>
            <p>Are you sure you want to {isDemotion ? 'demote' : 'promote'} all approved students from <strong>{fromLabel}</strong> to <strong>{toLabel}</strong>?</p>
            ...
            <p style={{ marginTop: 8 }}>New Academic Year will be set to <strong>{newAcademicYear || 'Auto'}</strong>.</p>
          </div>
        ),
        okText: `Yes, ${isDemotion ? 'Demote' : 'Promote'}`,
        okType: 'primary',
        cancelText: 'Cancel',
        onOk: async () => {
          try {
            ...
            setResult(res);
            message.success(`${res.updatedCount} students ${isDemotion ? 'demoted' : 'promoted'} successfully${res.newAcademicYear ? ` and academic year set to ${res.newAcademicYear}` : ''}!`);
            await loadSummary();
          } catch (err) {
            message.error(err?.response?.data?.message || `${actionLabel} failed`);
          } finally {
            setLoading(false);
          }
        },
          ...
          <div style={{ background: '#f0f4f8', borderRadius: 16, padding: 4, marginBottom: 24 }}>
            <div style={{ background: '#fff', borderRadius: 14, padding: 28 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
                ...
                <h4 style={{ fontFamily: "'Manrope', sans-serif", fontSize: 18, fontWeight: 800, color: '#00152a', margin: 0 }}>{mode === 'demote' ? 'Demote Students' : 'Promote Students'}</h4>
              </div>
              ...
              <Form form={form} layout="vertical">
                <Row gutter={16}>
                  ...
                  <Col span={8}>
                    <Form.Item label={<span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, color: '#43474d' }}>New Academic Year</span>}>
                      <Input
                ...
                <Button type="primary" icon={<SwapOutlined />} onClick={onPromote} loading={loading} size="large">
                  {mode === 'demote' ? 'Demote Students' : 'Promote Students'}
                </Button>
              </Form>
              ...
              {result && (
                <div style={{ marginTop: 24, padding: 20, background: '#f6fafe', borderRadius: 12 }}>
                  <Space direction="vertical" style={{ width: '100%' }}>
                    <span style={{ fontFamily: "'Manrope', sans-serif", fontWeight: 700, color: '#00152a', fontSize: 15 }}>{mode === 'demote' ? 'Demotion Result' : 'Promotion Result'}</span>
                    ...
                    <span style={{ color: '#171c1f', fontSize: 14 }}>Students {mode === 'demote' ? 'demoted' : 'promoted'}: <strong>{result.updatedCount}</strong></span>
                    <span style={{ color: '#171c1f', fontSize: 14 }}>New academic year: <strong>{result.newAcademicYear || newAcademicYear || '-'}</strong></span>
                    {(result.promotedStudents?.length > 0 || result.demotedStudents?.length > 0) && (
                      <Table
                        size="small"
                        dataSource={result.promotedStudents || result.demotedStudents || []}
                        rowKey="id"
                        pagination={false}
                        columns={[
          ...
          <div style={{ background: '#f0f4f8', borderRadius: 16, padding: 4 }}>
            <div style={{ background: '#fff', borderRadius: 14, padding: 28 }}>
              ...
              <Table
                ...
                columns={[
                  { title: 'Standard', dataIndex: 'standard', render: (v) => <span style={{ fontWeight: 700, color: '#00152a', fontFamily: "'Manrope', sans-serif" }}>{formatStandardLabel(v)}</span> },
